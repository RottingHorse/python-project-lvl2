    ...
    if isinstance(value, bool):
        ...
        for k, v in value.items():
            if isinstance(v, Dict):
                strings.append(
                    DIFF_STRING(whitespaces, INDENT, k, _to_str(v, depth + 1))
                )
            else:
                strings.append(
                    DIFF_STRING(whitespaces, INDENT, k, _to_str(v, depth)))
        strings.append(tail)
        return "\n".join(strings)
    return str(value)
...
def _format(diff_dict: Dict, depth: int) -> str:
    ...
    for key, diffs in sorted(diff_dict.items()):
        ...
        diff = diffs.get("diff")
        if status == NESTED:
            strings.extend([NESTED_STRING(whitespaces, INDENT, key,
            ...
            strings.extend([
                DIFF_STRING(whitespaces, REMOVED, key,
                            _to_str(old_value, depth + 1)),
                DIFF_STRING(whitespaces, ADDED, key,
                            _to_str(new_value, depth + 1)),
            ])
        else:
            strings.append(
                DIFF_STRING(whitespaces, status, key,
